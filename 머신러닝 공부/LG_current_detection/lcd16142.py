...
ch0_buf = deque(0 for _ in range(50))
ch1_buf = deque(0 for _ in range(50))
ch2_buf = deque(0 for _ in range(50))
ch3_buf = deque(0 for _ in range(50))
avg = 0
ch0_avg = 0
ch1_avg = 0
ch2_avg = 0
...
diff_ch0_ch1_buff = deque(0 for _ in range(50))
diff_ch0_ch1_avg = 0
...
def AddValues(val, ch_index):
    ...
    if ch_index == 0:
        global ch0_avg
        global ch0_buf
        ch0_avg = ch0_avg + 1*(val-ch0_avg)
        ch0_buf.append(ch0_avg)
        ch0_buf.popleft()
    elif ch_index == 1:
        global ch1_avg
        global ch1_buf
        ch1_avg = ch1_avg + 1*(val-ch1_avg)
        ch1_buf.append(ch1_avg)
        ch1_buf.popleft()
    elif ch_index == 2:
        global ch2_avg
        global ch2_buf
        ch2_avg = ch2_avg + 1*(val-ch2_avg)
        ch2_buf.append(ch2_avg)
        ch2_buf.popleft()
    elif ch_index == 3:
        global ch3_avg
        global ch3_buf
        ch3_avg = ch3_avg + 1*(val-ch3_avg)
